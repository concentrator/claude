...
# and trunk-committed plans/ tree stand in for the states the gate
# judges. Detection cases run with CI unset so the gate's own
# environment never skips them.
...
# Stale-batch fixture: a report committed on the trunk + the anchor tag.
mkstale() {
