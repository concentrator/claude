#!/usr/bin/env bash
# Tier-1 accretion gate (R-041, hardened by R-043): living plan artifacts
# state the present (writing.md § State the present). Flags dated
# supersession / amendment / status markers in tracked plan files under
# the artifacts root (resolve-root.sh) - the full ISO date is the
# discriminator: an undated terminal outcome ("mooted by R-021") is
# present state, a dated one ("superseded 2026-07-07") is hand-rolled
# version control that belongs to git history. A bare year never
# matches - alone it reads as a count, a key length, or an id. The
# separator tolerates bounded punctuation ("Superseded: 2026-07-07")
# but not sentence terminators. plans/archive/ is
