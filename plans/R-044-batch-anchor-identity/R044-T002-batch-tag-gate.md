...
- [ ] Stale-anchor detection: `scripts/ci/check-batch-tags.sh` fails on
...
- [ ] CI skip: `$CI` set or shallow clone → `check-batch-tags: SKIP
      (tags not visible)`, exit 0 - never a silent OK; test asserts the
      skip line both ways.
- [ ] Legacy form: any flat `pre-B-*` tag fails as unresolvable (no
      initiative to check a report against); test asserts.
- [ ] Doc sync: `DESIGN.md § Self-enforcement` and tree-map entries for
