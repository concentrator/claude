...
- [ ] `install-dev.sh`: ship `writing.md` into the target and append its `@import` to the target CLAUDE.md, idempotent + no clobber; `install-dev.test.sh` covers it (copied + import present, idempotent).
