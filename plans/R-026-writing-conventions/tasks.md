...
- [x] T-055 (R-026) [feat]: Em-dash Tier-1 check - `scripts/ci/check-no-em-dash.sh` over all tracked files + `run-all.sh` + pre-push + installer ship + `install-dev.test.sh` coverage + test
