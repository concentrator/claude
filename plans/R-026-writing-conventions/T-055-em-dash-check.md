...
- [ ] Test-first `scripts/test/check-no-em-dash.test.sh`: a planted em dash (built at runtime) in a `.md` fixture and in a `.sh` fixture both fail; a clean tree passes; a binary file is skipped. (red)
- [ ] Implement `scripts/ci/check-no-em-dash.sh`: scan tracked text files (`git ls-files`, `grep -I` to skip binary) for the em-dash character; report file:line + fail. (green)
