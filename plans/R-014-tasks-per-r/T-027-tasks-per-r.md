...
- [ ] Repoint rules + top-level structure docs to per-R `tasks.md`:
      ...
      stays green.
- [ ] Update `DESIGN.md`: planning-structure description + tree-map —
      per-R `tasks.md`, remove the root `TASKS.md` node. (architecture
      commit)
- [ ] Repoint the `~/.claude` skills to per-R `tasks.md`: `writing-plans`
...
- [ ] Update `migrating-to-dev`: repoint its `TASKS.md` references to
...
- [ ] Create the per-R `tasks.md` files: split the 26 tasks from
...
- [ ] Atomic swap: rewrite `scripts/ci/check-plan-integrity.sh` to read
