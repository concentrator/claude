...
- [x] Migration prompts (items 12-14; reviewer-prompt retire gated at close)
- [x] Close-review fixes: 10 half-applications completed - who-executes uniform across tbd/migrate/legacy, release mark decoupled from R-closure (rides every listed branch's final commit), batch prefix slug-exempt + merge-policy covered, release-branch prefix term, B-003 fossil, dead Conventions-key pointer.
- [x] Item 12 resolution (user): retire `plan-document-reviewer-prompt.md` - confirmed orphan, readiness review is user-run.: `plan-document-reviewer-prompt.md` - wire into the auto plan-review path with commit-checkbox vocabulary, or retire (user gate at close); `tbd-migration.md` - agent executes with user approval (migrate wins), `plans/tasks.md` naming, archival optionality restored; `legacy-migration.md` - § ID format pointer, REQ folds into foundational `REQUIREMENTS.md`, canonicalization via branch + MR/PR (no bootstrap exception).
