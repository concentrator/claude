...
- [ ] `agents/code-reviewer.md`: pin `model: fable`, add
...
- [ ] `branch-plan.md § Closing routine`: the three `/code-review`
...
- [ ] `delegation.md`: the close-review fan-out bullet is replaced by
...
- [ ] `verification-policy.md § Effort mechanics`: rewrite to state
  the present - agent frontmatter and the dispatch surface carry an
  effort key, so per-dispatch effort routing is real; the model table
  and capacity fallback stay
