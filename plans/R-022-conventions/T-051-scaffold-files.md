...
- [ ] `layout.md`: add the required baseline-files set for a new project — `.env.example`, `.gitignore` (with `.env` ignored), `README.md`, `CLAUDE.md`, and the `.claude/` structure; drop stack files (`.dockerignore`). Mark which are required at scaffold.
- [ ] `skills/dev/companions/`: add baseline seed templates — a `.gitignore` seed (incl. `.env`, `.claude/settings.local.json`) and an `.env.example` seed.
