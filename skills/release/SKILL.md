...
5. **Cut a short-lived branch.** `git checkout -b release/vX.Y.Z` — a doc branch for the CHANGELOG/notes, delivered by PR (no long-lived release line).
...
10. **Hand off.** `git push -u origin release/vX.Y.Z` → CI-gated PR → merge to `main`, then tag trunk: `git tag -a vX.Y.Z -m "<short>"` + `git push origin vX.Y.Z`. No release branch kept (`git-workflow.md § Releases`).
...
- Never edit `CHANGELOG.md` on the default branch directly — use the short-lived release PR.
