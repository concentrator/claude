...
4. **Docs** - per project `CLAUDE.md § Conventions`: if `release-routine: yes`, add `CHANGELOG.md ## [Unreleased]` entry (public-surface under existing kind headings; internal-only under `### Internal`); update `README.md` + extended docs only if surface changed. The feature's `.claude/docs/` doc is read at plan and reconciled at branch close (`branch-plan.md § Closing routine`), not per-commit.
