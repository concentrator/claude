...
- [ ] `companions/planner-prompt.md`: the dispatch template - inputs
  ...
  initiative's other plans; never a transcript or the planning
  conversation), the job (one branch plan per `write-plan.md`, or one
  change to an existing plan, stated as the diff of items), the exit
  (the cold read of `write-plan.md`, re-run until it passes, then
  `cold-read: passed`), and the report format.
- [ ] `write-plan.md` and `plan.md § Adjusting existing plans`: the
  ...
  approves, it no longer writes plan text itself. `§ Bulk mode` is the
  same dispatch in parallel.
- [ ] `run.md`: a worker's blocker that needs the plan changed, or a
  ...
  planner with the blocker's text, and resumes the worker on the
  re-read plan; `branch-plan.md § Rails` says no seat but the planner
  edits plan content, the implementer marking checkboxes only;
