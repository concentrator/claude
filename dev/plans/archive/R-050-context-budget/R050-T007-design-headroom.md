...
- [x] Audit `§ Self-enforcement` and `§ Git & delivery model` sentence by
...
- [x] Apply the trims whose owner is elsewhere, leaving a pointer where a
      ...
      its own header.
- [x] Verify the headroom is real, not just the count: add the three
