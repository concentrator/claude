...
- [ ] Define the two test tiers in `companions/declarations.md
...
- [ ] Point the per-commit Verify at the fast tier:
...
- [ ] Name the full tier in `finish.md`: § 1 Verify and the § 3 Ship
  gate run `Test (full)` once, CI on the MR/PR the authority.
- [ ] Align auto mode: `auto.md` pre-flight and member-branch close
  run the fast tier, batch close runs the full tier;
  `companions/implementer-prompt.md` Verify step names the fast tier.
- [ ] Declare the tiers in this repo's `CLAUDE.md § Agent toolchain`:
