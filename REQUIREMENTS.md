...
- Keep one directory per initiative (`R<NNN>-<slug>/` under the
  declared plans tree) holding its `requirements.md`, task index
  (`tasks.md`), branch plans (`<task-id>-<slug>.md`), findings, and
  batch manifests; the cross-initiative index (`ROADMAP.md`) lives at
  that tree's root; foundational docs at the repo root.
