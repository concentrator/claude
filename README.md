...
| `agents/` | The seat definitions of `/dev run`, one per dispatched seat - planner, cold reader, implementer, spec reviewer, doc writer, docs verifier, code reviewer: each holds the seat's tool set, its model where it pins one, and the conduct that holds on every dispatch; what a single dispatch hands the seat (inputs, exit, report format) stays under `skills/dev/`, and `skills/dev/run.md § Seats` lists both homes. This repository's own: `install-dev.sh` ships none of them |
