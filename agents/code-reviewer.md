...
The dispatcher will provide the plan path (typically a branch plan under the project's artifacts root, `plans/R-XXX-<slug>/`, or a section reference) along with the diff or commit range to review. Read the plan fully, extract the task list, and verify each implementation step against it. If no plan path is provided, ask the dispatcher for it before proceeding.
