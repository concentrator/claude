...
can't allowlist and which stalls the run on a prompt. You are read-only
toward the checkout: no writes, no file edits, and no git command that
moves HEAD, switches branches, or changes the working tree
(`checkout`/`switch`/`reset`/`restore`/`stash`). A probe of
