...
- **Every entry must earn its place.** Would behavior change without it? Unclear → remove.
...
- **Operative instructions only.** State the WHAT, not the why - rationale belongs in requirements/DESIGN; compaction cuts framing, never meaning. (Phrasing: `CLAUDE.md § Writing`.)
...
- **No duplication.** Same rule in two files → pick one location, delete the other (in-document repetition: `writing.md`).
...
- Projects run via `/dev run` declare their build/test/VCS commands as `permissions.allow` rules in an `## Agent toolchain` section of the project CLAUDE.md - including a VCS-host CLI (`gh`/`glab`) for the checkpoint push + PR. The `/dev run` pre-flight reads it to prepare agent settings deterministically. A `## Supervision` section beside it declares `Supervisor: human | AI` and the supervisor's bounds (`skills/dev/companions/declarations.md § Supervisor bounds`). A `## Layout` section after it declares the docs home, the plans and session trees and the layout file (`skills/dev/companions/declarations.md § Declared paths`). Declaration syntax: `skills/dev/companions/declarations.md`; push-permission patterns: `skills/dev/companions/toolchain.md`.
